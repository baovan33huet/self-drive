...
class RacingEnv:
    ...
    def __init__(self):
        ...
        self.back_image = pygame.image.load("resized_map4.png").convert()
    ...
    def get_state(self):
        ...
        # Lấy thông tin từ các cảm biến (rays)
        observations = self.car.cast(self.walls)  # cast trả về danh sách các cảm biến

        # Nếu `cast` trả về 19 giá trị, hãy đảm bảo rằng bạn không thêm thêm giá trị nào
        # Nếu `cast` trả về 18 giá trị, thêm vận tốc vào cho đủ 19 giá trị
        if len(observations) == 18:
            velocity = self.car.vel / self.car.maxvel  # Chuẩn hóa vận tốc
            observations.append(velocity)

        # Kiểm tra tổng số đặc trưng có đúng 19 không
        assert len(observations) == 19, f"Expected 19 features but got {len(observations)}"

        # Trả về trạng thái hoàn chỉnh (cảm biến + vận tốc)
        return np.array(observations)

    def reset(self):
        self.screen.fill((0, 0, 0))

        self.car = Car(437, 512)
        self.walls = getWalls()
        self.goals = getGoals()
        self.game_reward = 0
